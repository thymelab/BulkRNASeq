...
colData(ddsHTSeq)$batch <- factor(colData(ddsHTSeq)$batch)

threshold <- 0.6 
count_matrix = counts(ddsHTSeq)
#print(any(count_matrix < 0))
#count_matrix[count_matrix < 0] <- 0
keep_genes <- rowMeans(count_matrix > 0, na.rm = TRUE) > threshold
count_matrix <- count_matrix[keep_genes, ]


#count_matrix <- counts(ddsHTSeq)
mod <- model.matrix(~ batch + condition, data = colData(ddsHTSeq))
svobj <- svaseq(count_matrix, mod=mod)
corrected_counts <- ComBat_seq(count_matrix, batch = colData(ddsHTSeq)$batch)#, mod=mod)#svaobj = svobj)
...
#dds <- DESeq(ddsHTSeq, test="Wald", modelMatrix = ~ condition + batch)
#dds <- runCombat(dds, "batch")

#mod <- model.matrix(~ condition + batch, data = colData(ddsHTSeq))
#mod$batch <- as.factor(mod$batch)
#svobj <- svaseq(count_matrix, mod=mod)#, design = ~ condition + batch)
#corrected_counts <- ComBat(count_matrix, batch = mod$batch, mod = mod)
#corrected_counts <- ComBat(count_matrix, batch = colData(ddsHTSeq)$batch)


dds_correct <- DESeqDataSetFromMatrix(countData = corrected_counts,
                                        colData = colData(ddsHTSeq),
                                        design = ~ batch + condition)

dds <- DESeq(dds_correct)

#dds <- DESeq(ddsHTSeq, test="Wald", modelMatrix = ~ condition + batch + svobj$sv)
#dds <-DESeq(ddsHTSeq, test="Wald",  modelMatrix = ~ condition + batch, fitType = "parametric") #formula( ~ condition + batch))
#dds <-DESeq(ddsHTSeq, test="LRT", reduced = ~condition)
...
jpeg(file=paste0(outputPrefix,"_batchPCA.jpeg"))
plotPCA(rlog,"batch")
dev.off()
